...
class GPTAlibiAttention(nn.Module):
    ...
    def _attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if key_length > self.bias.shape[-1]:
         ...
         ##ALIBI
         # this is called position bias in mpt code
        alibi = self.build_mpt_alibi_tensor(self.num_attention_heads, self.config.max_position_embeddings, device=query.device)
        ...
        position_bias_query_index = max(0, alibi.size(1) - query_length)
        position_bias_key_index = max(0, alibi.size(2) - key_length)
        
        alibi = alibi[:, position_bias_query_index:, position_bias_key_index:]
        ...
        #print(f"ATTN WEIGHTS {attn_weights[0][0]}")
        #ferkn
...
class GPTNeoXAlibiLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention = GPTAlibiAttention(config)
...
class GPTNeoXAlibiModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
